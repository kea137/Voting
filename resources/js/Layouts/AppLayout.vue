        ...
        <div class="w-full px-2 md:px-0 md:w-175">
            <nav class="hidden md:flex items-center justify-between text-xs">
                <ul class="flex uppercase font-semibold border-b-4 pb-4 space-x-10">
                    <li><Link :href="route('ideas')" :class="{'border-blue-500': route().current('ideas')}" class="border-b-4 pb-4 hover:border-blue-500 transition ease-in duration-150">All Ideas({{ $page.props.all }})</Link></li>
                    <li><Link :href="route('considering')" :class="{'border-yellow-500': route().current('considering')}" class=" text-gray-600 transition duration-150 ease-in border-b-4 pb-4 hover:border-yellow-500">Considering({{ $page.props.considering }})</Link></li>
                    <li><Link :href="route('inprogress')" :class="{'border-green-500': route().current('inprogress')}" class=" text-gray-600 transition duration-150 ease-in border-b-4 pb-4 hover:border-green-500">In-Progress({{ $page.props.inprogress }})</Link></li>
                </ul>

                <ul class="flex uppercase font-semibold border-b-4 pb-4 space-x-10">
                    <li><Link :href="route('implemented')" :class="{' border-purple': route().current('implemented')}" class="text-gray-600 transition duration-150 ease-in border-b-4 pb-4 hover:border-purple">Implemented({{ $page.props.implemented }})</Link></li>
                    <li><Link :href="route('closed')" :class="{'border-red-500': route().current('closed')}" class=" text-gray-600 transition duration-150 ease-in border-b-4 pb-4 hover:border-red-400">Closed({{ $page.props.closed }})</Link></li>
                </ul>
            </nav>
            <div class="mt-8">
            <slot></slot>
